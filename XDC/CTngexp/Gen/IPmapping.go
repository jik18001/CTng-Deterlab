...
func Generate_IP_Json_template(num_ca int, num_logger int, num_monitor_gossiper int, ca_mask string, ca_offset int, logger_mask string, logger_offset int, mg_mask string, mg_offset int) IP_Json {
	...
	for i := 0; i < num_ca; i++ {
		ca_ip_map[i] = ca_mask + strconv.Itoa((i%10 + ca_offset))
	}
...
func Map_local_to_IP_priv_config(priv_config_path string, pub_config_path string, crypto_config_path string, entity_type string, ip_json_config IP_Json, entityID int, num_logger int, Threshold int) {
	switch entity_type {
	case "monitor":
		Map_local_to_Monitor_IP_priv_config(priv_config_path, ip_json_config, entityID, Threshold)
	case "gossiper":
		Map_local_to_Gossiper_IP_priv_config(priv_config_path, ip_json_config, entityID)
	case "ca":
		Map_local_to_CA_IP_priv_config(priv_config_path, ip_json_config, entityID, num_logger)
	case "logger":
...
func Map_local_to_Monitor_IP_priv_config(priv_config_path string, ip_json_config IP_Json, MID int, Threshold int) {
	...
	if MID > Threshold {
		monitor_priv_config.CA_URLs = make([]string, 0)
		monitor_priv_config.Logger_URLs = make([]string, 0)
	}
...
func Map_local_to_CA_IP_priv_config(priv_config_path string, ip_json_config IP_Json, MID int, num_logger int) {
	...
	Logger1 := ca_priv_config.Loggerlist[0]
	Logger2 := ca_priv_config.Loggerlist[1]
	//change localhost to actual IP
	//take the data out and slice out the last 5 characters
	semicolon_and_port_L1 := Logger1[len(Logger1)-5:]
	new_dest_1 = ip_json_config.Logger_ip_map[(MID-1)%num_logger] + semicolon_and_port_L1
	ca_priv_config.Loggerlist[0] = new_dest_1
	semicolon_and_port_L2 := Logger2[len(Logger2)-5:]
	new_dest_2 := ip_json_config.Logger_ip_map[num_logger-((MID-1)%num_logger)-1] + semicolon_and_port_L2
	ca_priv_config.Loggerlist[1] = new_dest_2
...
func Map_all(num_ca int, num_logger int, num_mg int, Threshold int, ip_json_config IP_Json) {
	...
	for i := 0; i < num_ca; i++ {
		...
		Map_local_to_IP_priv_config(ca_privconfig_path, ca_pubconfig_path, ca_cryptoconfig_path, "ca", ip_json_config, i+1, num_logger, Threshold)
	...
	for i := 0; i < num_logger; i++ {
		...
		Map_local_to_IP_priv_config(logger_privconfig_path, logger_pubconfig_path, logger_cryptoconfig_path, "logger", ip_json_config, i+1, num_logger, Threshold)
	...
	for i := 0; i < num_mg; i++ {
		...
		Map_local_to_IP_priv_config(monitor_privconfig_path, monitor_pubconfig_path, monitor_cryptoconfig_path, "monitor", ip_json_config, i+1, num_logger, Threshold)
	...
	for i := 0; i < num_mg; i++ {
		...
		Map_local_to_IP_priv_config(gossiper_privconfig_path, gossiper_pubconfig_path, gossiper_cryptoconfig_path, "gossiper", ip_json_config, i+1, num_logger, Threshold)
